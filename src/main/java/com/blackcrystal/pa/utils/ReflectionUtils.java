...
public abstract class ReflectionUtils {
	...
	public static List<Field> getAllFields(Class<?> clazz,
			Class<? extends Annotation> annotation, boolean excludeStaticFileds)
			throws Exception {
	...
	public static Method getPublicMethod(Class<?> clazz, String name,
			Class<?> signatures[]) throws NoSuchMethodException {
		Method r = findPublicMethod(clazz, name, signatures);
		if (r == null) {
			throw new NoSuchMethodException(clazz.getName() + "." + name
					+ ArrayUtils.toString(signatures));
		}
		return r;
	}

	public static Method findPublicMethod(Class<?> clazz, String name,
			Class<?> signatures[]) {
		...
		for (Method method : clazz.getMethods()) {
			if (method.getName().equals(name)) {
				if (matchArguments(signatures, method.getParameterTypes(),
						false)) {
					methods.add(method);
				}
			}
	 ...
	 * 
	 */
	private static Method getMostSpecificMethod(List<Method> methods,
			Class<?> signatures[]) {
		//
		int maxMatches = 0;
		Method method = null;
		for (Method m : methods) {
			...
			if (matches == 0 && maxMatches == 0) {
				if (method == null) {
					...
					// If the current method parameters is higher in the
					// inheritance hierarchy then replace it.
	...
	private static boolean matchArguments(Class<?> signatures[],
			Class<?> paramTypes[], boolean explicit) {
		//
		...
		for (int j = 0; j < signatures.length; j++) {
			...
			//
			if (explicit) {
				if (signatures[j] != paramType) { // Test each element for
													// equality
					return false;
				}
			} else { // Consider null an instance of all classes.
				if (signatures[j] != null
						&& !(paramType.isAssignableFrom(signatures[j]))) {
					return false;
				}
			}
		}
